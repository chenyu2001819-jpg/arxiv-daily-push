...
class KeywordManager:
    ...
    def _load_keywords(self):
        ...
        for line in lines:
            ...
            # 检测分组标题（以"关键词"或"扩展"结尾的行视为分组标题）
            if line.endswith('关键词') or '扩展' in line:
                current_group = line
                self.keyword_groups[current_group] = []
                continue
            
            # 处理一行多个关键词的情况（用空格或顿号分隔）
            sub_keywords = re.split(r'[\s、]+', line)
            for kw in sub_keywords:
                kw = kw.strip()
                if kw and len(kw) > 1:  # 过滤单字关键词
                    self.keywords.append(kw.lower())
                    self.keyword_groups[current_group].append(kw.lower())
        
        # 去重
        self.keywords = list(set(self.keywords))
        logger.info(f"加载了 {len(self.keywords)} 个关键词，分为 {len(self.keyword_groups)} 组")
    
    def get_search_queries(self) -> List[str]:
        """生成 arXiv 搜索查询词（英文关键词需要翻译或映射）"""
        # 中英文关键词映射表（可根据需要扩展）
        translations = {
            # 产业组织
        ...
        for kw in self.keywords:
            ...
            elif kw.isascii():  # 已经是英文
                queries.append(kw)
            # 中文关键词没有翻译的暂时跳过（arXiv主要是英文）
...
class PaperRanker:
    ...
    # 高权重分类（经济学、能源、环境、交通相关）
    HIGH_WEIGHT_CATEGORIES = {
        'econ', 'q-fin', 'cs.CE', 'cs.CY',  # 经济和金融
        'physics.ao-ph', 'physics.geo-ph',   # 大气和地球物理
        'stat', 'math.OC',                    # 统计和优化
    }
...
class ArxivAgent:
    ...
    def _load_config(self, config_file: str) -> Dict:
        ...
        # 然后加载环境变量配置（优先级更高，用于 GitHub Actions）
    ...
    def _get_paper_id(self, paper: Paper) -> str:
        """生成文章唯一ID"""
        # 从链接中提取 arXiv ID
        # 链接格式: http://arxiv.org/abs/2401.12345
        link = paper.link
        if '/abs/' in link:
            return link.split('/abs/')[-1].split('v')[0]  # 移除版本号
        return paper.title[:50]  # 备用方案
    ...
    def run(self, send_email: bool = True) -> str:
        ...
        # 2. 搜索文章
        all_papers: Dict[str, Paper] = {}  # 使用字典去重
        ...
        logger.info(f"最终选择 {len(selected_papers)} 篇文章（阈值: {min_score}）")
    ...
    def _generate_report(self, papers: List[Paper]) -> str:
        ...
        for paper in papers:
            ...
            elif any(kw in matched_text for kw in ['市场', '产业', '竞争', '定价', 'market', 'industr', 'competition']):
